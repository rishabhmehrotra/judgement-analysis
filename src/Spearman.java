...
public class Spearman {
	...
	public static void main(String[] args) throws IOException {
		...
		while(suspectRemains == 1)
		{
			...
			for(int ttt=1;ttt<=nItr;ttt++)
			{
				...
				while(line!=null)
				{
					...
					if(!validJudges.containsKey(jid1)) validJudges.put(jid1, jid1);
					if(!validJudges.containsKey(jid2)) validJudges.put(jid2, jid2);
					if(!validJudges.containsKey(jid3)) validJudges.put(jid3, jid3);
					if(!validJudges.containsKey(jid4)) validJudges.put(jid4, jid4);
					if(!validJudges.containsKey(jid5)) validJudges.put(jid5, jid5);
			...
			double maxP=0.0;int maxJ=-1;
			for(int i=0;i<246;i++)
			{
				if(!validJudges.containsKey(i)) continue;
				double n = perJudgePValueNum.get(i);
				double pval = n/nItr;
				if(pval>maxP){maxP=pval;maxJ=i;}
			}
			if(maxP>0.1) {suspects.put(maxJ, maxJ);System.out.println("Added to suspect list...judge "+maxJ+" with pval= "+maxP+" size of suspects: "+suspects.size());}
			for(int i=0;i<246;i++)
			{
				if(!validJudges.containsKey(i)) continue;
				double n = perJudgePValueNum.get(i);
		...
		for(int i=0;i<246;i++)
		{
			...
			double n = perJudgePValueNum.get(i);
			double pval = n/nItr;
			System.out.println(i+" "+pval+" "+perJudgeAverages.get(i)+" "+suspects.containsKey(i));
		}
		System.out.println("Valid judges: "+validJudges.size());
		System.out.println("Suspect Judges: "+suspects.size());
